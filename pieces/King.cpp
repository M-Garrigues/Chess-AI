    ...
    for(int i = x-1; i < x+2;i++){
        for(int j = y-1; j < y+2; j++){
            ...
            if(i!=x || j != y){
                ...
                //std::cout << "x = " << i << "  y = " << j << "  threat = " << threats[i][j] << "  bd = " << bd[i][j] << std::endl;
                if(bd[i][j] == 0){

                    if(threats[i][j] == 0){
                        cells.push_back(i+(j-1)*8);
                    }
                }else if(bd[i][j] > -10 && bd[i][j] < 10){

                    if(threats[i][j] == 0 && !board.getPiece(i+(j-1)*8)->getColor() == getColor()){
                        cells.push_back(i+(j-1)*8);
                    }
                }
            }
...
std::vector<int> King::getAttackCells(Board &board) {
    return King::getMoveCells(board);
}
